...
class BMG:
  ...
  def _initFromData(self, data: bytes) -> None:
    ...
    magic, dataLen, sectionCount, enc, self.unk14, self.unk18, self.unk1C = struct.unpack_from(se + "8sIIB3I", data, 0)
    ...
    def parseFLW1(offset: int, length: int) -> None:
      instructionsCount, labelsCount, unk0C = struct.unpack_from(se + "HHI", data, offset + 8)
    ...
    def parseFLI1(offset: int, length: int) -> None:
      count, entryLength, unk0B, unk0C = struct.unpack_from(se + "HBBI", data, offset + 8)
    ...
    for i in range(sectionCount):
      ...
      if sectionMagic == b"INF1" or sectionMagic == b"1FNI":
        parseINF1(offset, sectionLen)
      elif sectionMagic == b"DAT1" or sectionMagic == b"1TAD":
        parseDAT1(offset, sectionLen)
      elif sectionMagic == b"FLW1" or sectionMagic == b"1WLF":
        parseFLW1(offset, sectionLen)
      elif sectionMagic == b"FLI1" or sectionMagic == b"1ILF":
    ...
    for offset, attribs in INF1:
      ...
      while nextBytes != nullChar:
        if nextBytes == escapeSequenceStart:  # escape sequence
          ...
          stringParts.append(Message.Escape(escapeType, escapeData, self.magic_endianness))
          offset += escapeLen
          currentStringStart = offset
        else:
          offset += len(nullChar)
  ...
  def save(self) -> bytes:
    ...
    se = self.endianness
    if se not in "<>":
      raise ValueError(f"BMG.endianness is '{se}', which is neither '<' nor '>'")
    if not isinstance(self.encoding, str) or self.encoding.lower() not in _ENCODINGS:
    ...
    for i, message in enumerate(self.messages):
      if len(message.info) != inf1EntryLen - 4:
        raise ValueError(
          f"Message info values are presumed to"
  ...
  def __str__(self) -> str:
    return f"<bmg id={self.id} ({len(self.messages)} messages, {len(self.scripts)} scripts)>"
  ...
  def __repr__(self) -> str:
    ...
    return f"{type(self).__name__}.fromMessages({', '.join(args)})"
...
class Message:
  ...
  class Escape:
    ...
    def __init__(self, type: int = 0, data: bytes = b"", endianess: str = ">"):
      self.type = type
      self.data = data
